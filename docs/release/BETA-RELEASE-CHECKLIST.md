...
| G1.8 | Animation stress p95 ≤ 16.6 ms **on release-class hardware** (B7). Hosted CI runners are shared, so treat their timings as indicative — `--mode cpu` (the only mode that runs `test-animation-stress`; `--mode gpu` doesn't touch this suite) now runs it advisory (`VIEWW_TIMINGS_ADVISORY=1`: measures and records to `metrics.txt`, does not fail the gate) precisely because a shared runner can't stand in for release-class hardware. **This checkbox is only satisfied by running `test-animation-stress` (directly, or via `ci/vieww gate --mode cpu`) by hand on a chosen minimum-spec machine and reading its real numbers** — a green hosted-CI gate is not evidence for it, advisory or not. | ☐ | N/A (CPU work) | ☐ | N/A (CPU work) | ☐ | N/A (CPU work) |
...
| B7 | P1 | `test-animation-stress` misses its 16.6 ms budget. p95 was 24 ms on a 2-core container; on the first real run (Linux, i5-7200U, 2017 laptop) it was **35 ms**, with 72 of 72 frames over budget. Re-profiled (manual `Instant` instrumentation; no `perf` in the profiling environment) on a 2-core container: of the ~21 ms/frame over budget, the CPU renderer's shadow cache accounts for **~8.5 ms** — `ShadowKey` (`crates/vieww-paint/src/native/shadow.rs`) includes the shadow's absolute `rect` position, so every moving/animated shadow is a 100% cache miss every frame — and background-fill anti-aliasing accounts for **~3.7 ms** (one `FillRect` per frame, unbatched). Together, over half the overage, with the rest spread thinly across the other 100-tile draw commands. | Two independent problems live under one budget. (1) The 16.6 ms budget is a release-class-hardware claim (see G1.8); a shared 2-core CI container was never going to meet it, and treating a miss there as a build failure was the actual bug. Fixed: `ci/release/gate.sh`'s `--mode cpu` now sets `VIEWW_TIMINGS_ADVISORY=1`, the example's own existing (previously unwired) escape hatch — a CPU-mode timing miss is measured to `metrics.txt` and recorded by `ci/certify/certify.sh` as `PASS(ADVISORY: ...)`, not `FAIL`. This does not, and cannot, make hosted CI certify G1.8: `test-animation-stress` doesn't run under `--mode gpu` either (`VIEWW_CERT_ONLY=gpu` skips every CPU-rendered suite, including this one) — no hosted mode actually exercises release-class hardware. G1.8 stays checked off only by running the suite by hand on a chosen minimum-spec machine (see G1.8's own note). (2) The renderer-side cost (shadow cache keyed on absolute position; unbatched background fill) is untouched by this pass — it is correctness-sensitive code with extensive golden-image coverage and deserves a scoped, reviewed fix rather than a blind one. Tracked as follow-up: key `ShadowKey` on the shadow's local geometry instead of its absolute position (invalidate/translate the cached mask instead of recomputing it), and consider batching the background fill. Re-measure G1.8 on real release-class hardware once a minimum supported CPU is chosen; the CI change does not itself change what ships. | Open — floor documented, CI no longer conflates a shared-runner reading with a release-hardware failure; renderer fix is a scoped follow-up |
...
| B11 | **P0** | **Desktop app segfaulted on real Linux (Wayland, Intel Mesa)** during the desktop suite. Cause: window teardown destroyed the winit window and the Vulkan device while the swapchain and `VkSurfaceKHR` were still alive (Vulkan validation: `VUID-vkDestroyDevice-device-05137`, `VUID-vkDestroyInstance-instance-00629`). Any closing dialog or app exit could crash. | **Fixed** in this tree (`Drop for Gpu` releases the swapchain first). Validation errors are now 0 on X11 and Wayland. The fix landed undocumented and without a regression test — a stale module comment in `vieww-platform-winit/src/native.rs` still said "No multi-window device sharing", and `wait_loop.rs`'s `two_windows_share_one_gpu_device` scenario had been reduced to a comment. Both fixed: the comment now matches the code, and the test is restored (asserts one shared `VulkanDevice` — same identity, `device_opens_this_process()` unchanged — across two windows opened in one process) and passing under Xvfb + lavapipe; sabotaging `shared_device_for` to skip reuse reliably fails it. Confirm with G1.10 on the original Wayland/Intel Mesa laptop. | Fixed, awaiting re-run on the original hardware — root fix, doc, and regression test now all in tree |
| B12 | P1 | Vieww Standard startup took 52.7 ms against a 33.3 ms budget on the i5-7200U. The same run measured the **system font scan at 46 s**. Every window runs that scan synchronously before its first frame (`use_system_fonts`), so Studio may take tens of seconds to open on slower disks. | **Fixed**: `vieww-text/src/font.rs`'s `with_system_fallback()` scanned the system font database from scratch for every `FontStore`, i.e. every window — so a multi-window app paid the ~46 s scan repeatedly, once per window. It now builds the `fontdb::Database` once per process behind a `OnceLock` (`scanned_system_db()`) and clones the cheap `Database` handle for each window thereafter. Added `opening_several_windows_scans_the_system_fonts_once` (asserts an internal scan counter stays at 1 across 4 `FontStore`s) — `cargo test --release -p vieww-text` passes (118 tests, 0 warnings). Still measure Studio launch time (G6.1) on the i5-7200U: this fixes the *repeat*-scan cost, not the ~46 s cost of the single first scan, which a slow disk still pays once at first launch. | Fixed, awaiting re-run — repeat-scan cost eliminated and regression-tested; first-launch scan cost is unchanged and outside this fix's scope |
